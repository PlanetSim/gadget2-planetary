...
#define BLKLEN fwrite(&blklen, sizeof(blklen), 1, fd);
  ...
  for(i=1; i <= num; i++)
    {
      if(p1->bnd[i] != 1)
	fwrite(&p1->Pr[i], sizeof(float), 1, fd);
    }
  BLKLEN;
  ...
  for(i=1; i <= num; i++)
    {
      if(p1->bnd[i] != 1)
	fwrite(&p1->Te[i], sizeof(float), 1, fd);
    }
  BLKLEN;
  ...
  for(i=1; i <= num; i++)
    {
      if(p1->bnd[i] != 1)
	fwrite(&p1->U[i], sizeof(float), 1, fd);
    }
  BLKLEN;
  ...
  for(i=1; i <= num; i++)
    {
      if(p1->bnd[i] != 1)
	fwrite(&p1->cs[i], sizeof(float), 1, fd);
    }
  BLKLEN;
  ...
  for(i=1; i <= num; i++)
    {
      if(p1->bnd[i] != 1)
	fwrite(&p1->accel[i][1], sizeof(float), 3, fd);
    }
  BLKLEN;
  ...
  for(i=1; i <= num; i++)
    {
      if(p1->bnd[i] != 1)
	fwrite(&p1->dt[i], sizeof(float), 1, fd);
    }
  BLKLEN;
