...
int main( int argc, char **argv ) {
  ...
  isobars = malloc( nS * nRho * sizeof(double) );
  pressures = malloc( nRho * sizeof(double) );
  ...
  for( i = 0; i < nRho; i++ ) {
	...
	isobars[ i*nS + 0 ] = T[ S_zero*nRho + i ];
      } else {
	//The general case
  ...
  save_table( tablefilename, isobars, S, pressures, nS, nRho );
...
void save_table( char * file, double * isobars, double * S, double * pressures, int nS, int nRho ) {
  ...
  for( maxindex = 0; maxindex < (nRho*nS); maxindex++ ) {
    if( pressures[maxindex] >=  _MAXPRESSURE ) {
      break;
    }
  }
  maxindex--;
