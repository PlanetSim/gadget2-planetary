...
def read_params( file ):
    ...
    return  bellfiles, critpoints, inputdir, snapbase
...
def main():
    ...
    bellfiles, critpoints, inputdir, snapbase = read_params( paramfile )
    bellcurves = []
    for i in range( len(bellfiles) ):
        bellcurves.append( get_bell_curve( bellfiles[i] ) )
    dirlist = os.listdir( inputdir )
    print "Analysing approx. "+str(len(dirlist))+" files."
    sys.stdout.write("[")
    sys.stdout.flush()
    count = 1
    for file in dirlist:
        if not( file[:len(snapbase)] == snapbase ):
            continue
        thermdata, tablelines = open_table( inputdir+file )
        add_mass_fractions( bellcurves, thermdata, critpoints, tablelines, inputdir+file )
