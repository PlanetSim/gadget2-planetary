		...
		if(Exportflag[j])
		  {
		    ...
		    /* calculation of F1 */
/* RAM & JAB, 06/13/07, Replaced sound speed computation.
		    soundspeed_i = sqrt(GAMMA * SphP[i].Pressure / SphP[i].Density);
*/
		    soundspeed_i = eos_table_interp(SphP[i].Density, SphP[i].Entropy, P[i].ID, EOS_SOUND_SPEED, EOS_EXTRAP);
  ...
  for(i = 0; i < N_gas; i++)
    if(P[i].Ti_endstep == All.Ti_Current)
      {
/* JAB & RAM, 06/14/07, Changed equation for p/rho^gamma --> real entropy
	SphP[i].DtEntropy *= GAMMA_MINUS1 / (hubble_a2 * pow(SphP[i].Density, GAMMA_MINUS1));
*/
	  SphP[i].DtEntropy /= eos_table_interp(SphP[i].Density, SphP[i].Entropy, P[i].ID, EOS_TEMPERATURE, EOS_EXTRAP); 
...
*/
      soundspeed_i = eos_table_interp(SphP[target].Density, SphP[target].Entropy, P[target].ID, EOS_SOUND_SPEED, EOS_EXTRAP);
...
/* RAM & JAB, 06/13/07, Replaced sound speed computation.
      soundspeed_i = sqrt(GAMMA * pressure / rho);
*/
      soundspeed_i = eos_table_interp(HydroDataGet[target].Density, HydroDataGet[target].Entropy, HydroDataGet[target].ID, EOS_SOUND_SPEED, EOS_EXTRAP);
...
*/
		  soundspeed_j = eos_table_interp(SphP[j].Density, SphP[j].Entropy, P[j].ID, EOS_SOUND_SPEED, EOS_EXTRAP);
