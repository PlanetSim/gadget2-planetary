      ...
      if(P[i].Ti_endstep == All.Ti_Current)
	{
	  ...
	  if(P[i].Type == 0)	/* SPH stuff */
	      ...
	      for(j = 0; j < 3; j++)
		{
	      ...
	      if(SphP[i].DtEntropy*dt_entr > 10.0 * SphP[i].Entropy){
		SphP[i].DtEntropy = 10.0*SphP[i].Entropy/dt_entr;
		//printf("Changed entropy for particle %d\n", P[i].ID);
		fflush(stdout);
	      }

	      // set DtEntropy to zero and set the new value of the entropy to the
	      // value that keeps the temperature fixed

	      SphP[i].DtEntropy = 0.0;

	      rad = sqrt(P[i].Pos[0]*P[i].Pos[0]+P[i].Pos[1]*P[i].Pos[1]+P[i].Pos[2]+P[i].Pos[2]);
	      if(first) SphP[i].initT = eos_table_interp(SphP[i].Density, SphP[i].Entropy, P[i].ID, EOS_TEMPERATURE, EOS_EXTRAP);


	      // now look up the new entropy from the table that has rho/T as independent variables
	      // entropy is in the third block of the rho/T table (the energy block for the rho/S table)
	      SphP[i].Entropy = eos_table_interp(SphP[i].Density, SphP[i].initT, P[i].ID+400000000, EOS_INT_ENERGY, EOS_EXTRAP);
				   ...
				   aphys */ )
{
  ...
  if(P[p].Type == 0)
    {
/* RAM & JAB, 06/13/07, Replaced the sound speeed computation.
      csnd = sqrt(GAMMA * SphP[p].Pressure / SphP[p].Density);
*/
      csnd = eos_table_interp(SphP[p].Density, SphP[p].Entropy, P[p].ID, EOS_SOUND_SPEED, EOS_EXTRAP);
