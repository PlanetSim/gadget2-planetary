...
void compute_global_quantities_of_system(void)
{
  ...
  for(i = 0; i < NumPart; i++)
      ...
      if(P[i].Type == 0)
	{
          ...
          egyspec = entr;
#else
/* JAB & RAM, 06/14/07, Replaced p/rho^gamma-->u conversion with eos table interpolation of internal energy given density and entropy. 
	  egyspec = entr / (GAMMA_MINUS1) * pow(SphP[i].Density / a3, GAMMA_MINUS1);
*/
	  egyspec = eos_table_interp(SphP[i].Density, entr, P[i].ID, EOS_INT_ENERGY, EOS_EXTRAP);
#endif
