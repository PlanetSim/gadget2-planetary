...
void init(void)
{
  ...
  {
      fprintf(stderr, "Error in init(): Initial conditions must contain entropy and flag_entropy_instead_u must be set to 1\n");
      exit(13);
  }
/*
    for(i = 0; i < N_gas; i++)
  RAM & JAB, Change the energy to entropy conversion so that entropy is the TRUE entropy.  We are doing this to eliminate the ICs as a possible source of error 
     SphP[i].Entropy = GAMMA_MINUS1 * SphP[i].Entropy / pow(SphP[i].Density / a3, GAMMA_MINUS1);

	SphP[i].Entropy = (GAS_CONST/GAMMA_MINUS1) * log(GAMMA_MINUS1 * SphP[i].Entropy / pow(1.0, GAMMA_MINUS1));

	RAM & JAB, 7/19/07, We want to exit with an error if the ICs contain the energy, because we cannot get the entropy from the energy for a solid.
*/
#endif
}
