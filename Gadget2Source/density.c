...
/*  RAM & JAB, 06/13/07, Replaced the pressure calculation.
		SphP[i].Pressure =
		  (SphP[i].Entropy + SphP[i].DtEntropy * dt_entr) * pow(SphP[i].Density, GAMMA);
*/

	        SphP[i].Pressure = eos_table_interp(SphP[i].Density, (SphP[i].Entropy + SphP[i].DtEntropy * dt_entr), P[i].ID, EOS_PRESSURE, EOS_EXTRAP);
	      }
